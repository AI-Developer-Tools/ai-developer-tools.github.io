...
Most projects that are written in Javascript or TypeScript should use the [Jest](https://jestjs.io) unit testing framework. If you are building a VS Code extension, you will use VS Code's preferred testing framework, Mocha. See [VS Code's Extension Testing Instructions](https://code.visualstudio.com/api/working-with-extensions/testing-extension) on how to set it up.
...
Generate unit tests for each test specification. At the end, you should have four test files. 
...
!!! note
    You will be graded on how well you prevent the LLM from hallucinating nonsensical test cases or creating duplicate or significantly overlapping test cases. The LLM must not generate test cases for functions that do not exist. 
...
How did it go? Did every test pass? If not, use your LLM to give you a plan on how it wants to fix the bugs (ask it for several alternative fixes). Choose the bug fix you like and have the LLM make the change. Did your test case pass? Congratulations! If not, try again.
...
Please turn in a single document that contains these parts:  
  ...
  1. The frontend test specification (1) for code file 1.
  1. The frontend test specification (2) for code file 2.
  1. The backend test specification (3) for code file 3.
  1. The backend test specification (4) for code file 4.
  1. Provide a Git repository link to the test code file (1) that implements frontend test spec 1.
  1. Provide a Git repository link to the test code file (2) that implements frontend test spec 2.
  1. Provide a Git repository link to the test code file (3) that implements backend test spec 3.
  1. Provide a Git repository link to the test code file (4) that implements backend test spec 4.
  1. Copy-paste in the test output from running frontend test file 1.
  1. Copy-paste in the test output from running frontend test file 2.
  1. Copy-paste in the test output from running backend test 3.
  1. Copy-paste in the test output from running backend test 4.
  1. Copy-paste in the code coverage report for frontend test file 1.
  1. Copy-paste in the code coverage report for frontend test file 2.
  1. Copy-paste in the code coverage report for backend test 3.
  1. Copy-paste in the code coverage report for backend test 4.
